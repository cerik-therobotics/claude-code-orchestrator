...
Before implementing a new feature, use Antigravity to research best practices.
...
4. Suggested areas for improvement" --add-dir src --add-dir lib --add-dir tests 2>/dev/null
...
- Response formatting" --add-dir src 2>/dev/null
```
...
# Code review video
agy -p "Extract code patterns and best practices demonstrated in this video" < code-review.mp4 2>/dev/null
```
...
agy -p "Extract technical insights from this talk about {topic}" < conference-talk.mp3 2>/dev/null
```
...
agy -p "Find and summarize the latest React 19 features and migration guide from official docs" 2>/dev/null
...
- Suggest migration order" --add-dir src 2>/dev/null
...
- Required changes" --add-dir . 2>/dev/null
```
...
```bash
agy -p "List all API endpoints in this codebase with their HTTP methods. You MUST return ONLY valid JSON. Do not include markdown, code fences, or any explanatory text." \
  2>/dev/null
```
...
agy -p "Generate comprehensive documentation for src/auth/" \
  --add-dir src/auth > docs/auth-module.md 2>/dev/null
```

## Rate Limits

Rate limits depend on the Antigravity account, selected model, and organization policy.
